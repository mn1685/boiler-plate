...
export const enrichMetadata = async (html, slug) => {
    try {
        ...
        const $ = load(html);
        $('title').text(`${presentation.title} | CaddyWizardAI`);
        $('meta[name="description"]').attr(
            'content',
            presentation.description ||
                `An engaging presentation created with CaddyWizardAI${
                    Array.isArray(presentation.slides) && presentation.slides.length
        ...
        $('meta[property="og:description"]').attr(
            'content',
            presentation.description || 'An engaging presentation created with CaddyWizardAI.'
        );
        $('meta[property="og:url"]').attr(
            'content',
            `https://CaddyWizardAI/presentation/${presentation.slug}`
        );
        ...
        if (presentation.theme && presentation.theme.imageUrl) {
            ...
            imageUrl = 'https://CaddyWizardAI/image2.jpg';
        }
